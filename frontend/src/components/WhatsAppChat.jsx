...
function WhatsAppChat({
  ...
  const handleSend = async () => {
    ...
    try {
      const res = await fetch(`${backendUrl}/api/channels/whatsapp/send`, {
        method: 'POST',
        headers: {
      ...
      console.error('[WhatsAppChat] Send error:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="flex flex-col flex-1 h-full bg-[#121212] overflow-hidden relative">
      {/* Header */}
        ...
        {activeConversation ? (
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-xs text-zinc-400">{activeConversation.peerId}</span>
            </div>
          </div>
        ) : (
          <div className="flex items-center gap-2">
            <Phone className="h-5 w-5 text-[#25d366]" />
            <span className="text-sm font-semibold tracking-wide text-zinc-100">WhatsApp Channel</span>
          </div>
        )}

        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <span className={`h-2.5 w-2.5 rounded-full ${whatsappStatus?.connected ? 'bg-green-500 shadow-md shadow-green-500/25' : 'bg-red-500'}`} />
            {whatsappStatus?.connected ? (
              <span className="text-xs font-semibold px-2.5 py-1 bg-zinc-800 rounded-lg text-zinc-300 max-w-[150px] sm:max-w-xs truncate" title={whatsappStatus.phoneNumber}>
                {whatsappStatus.phoneNumber || 'Unknown'}
              </span>
            ) : (
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4 space-y-3">
        {activeConversation ? (
          <div className="max-w-3xl mx-auto space-y-3">
            ...
            {typing && (
              <div className="flex justify-start">
                <div className="flex items-center gap-1 bg-[#202c33] rounded-xl rounded-tl-none px-4 py-3">
          ...
          <div className="flex flex-col items-center justify-center h-full max-w-md mx-auto text-center space-y-4 select-none">
            ...
            <h2 className="text-xl font-bold text-white">QueryDesk WhatsApp Dashboard</h2>
        ...
        <div className="p-4 bg-[#1f1f1f] border-t border-[#2d2d2d]">
          <div className="max-w-3xl mx-auto flex items-end gap-3">
            <textarea
              ref={textareaRef}
              rows={1}
              className="flex-1 rounded-xl bg-[#262626] border border-[#3f3f46] px-4 py-3 text-sm text-white placeholder-zinc-500 focus:outline-none focus:border-[#25d366] resize-none overflow-hidden"
            ...
            <button
              ...
              className={`shrink-0 p-3 rounded-xl transition duration-150 ${
                input.trim() && !loading
                  ? 'bg-[#005c4b] text-white hover:brightness-110'
